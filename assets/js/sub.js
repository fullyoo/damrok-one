$(function () {
    ...
    **************************************/
    ...
    function initSlider() {
        ...
        let currentSlide = 0;
        const slideTexts = []; // 필요하면 텍스트 추가

        // bars_container가 없으면 생성
        if ($(".progress_ctn .bars_container").length === 0) {
            $(".progress_ctn").append('<div class="bars_container"></div>');
        }
        const bars = $(".progress_ctn .bars_container");

        // 진행바 상태 업데이트
        const updateBars = (i) => {
            bars.find(".bar").removeClass("active completed");
            bars.find(".bar").eq(i).addClass("active");
            bars.find("span").css({ width: 0, opacity: 1 });
        };

        // 진행바 애니메이션
        const startProgress = (duration) => {
            const bar = bars.find(".bar.active span");
            bar.stop().animate({ width: "100%" }, duration, "linear", () => {
        ...
        // 슬라이드 이동
        const goToSlide = (i) => {
            bars.find("span").stop(true);
            currentSlide = i;
            updateBars(i);
            slider.slick("slickGoTo", i);
            setTimeout(() => startProgress(autoplaySpeed), 80);
        };

        // slick 초기화
        slider.on("init", function (e, slick) {
            // slick 원본 슬라이드 개수 가져오기
            ...
            for (let i = 0; i < total; i++) {
                bars.append(`
                    <div class="bar" data-slide="${i}">
                        <span></span>
                        <div class="bar-text">${slideTexts[i] || ""}</div>
                    </div>
                `);
            }
        ...
        $(".play_btn .stop").on("click", function () {
            const bar = bars.find(".bar.active span");
            const total = bars.find(".bar.active").width();
            ...
            if (!$(this).hasClass("on")) {
                ...
                const current = parseInt(bar.css("width"));
                const remain = autoplaySpeed * (1 - current / total);

                bar.stop().animate({ width: total }, remain, "linear")
